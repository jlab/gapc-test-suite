...
signature sig (alphabet, answer) {
	...
	answer f5 (answer);
	answer f6 (Rope);
	answer f7 (alphabet, answer, alphabet);
	answer f8 (Rope);
...
algebra alg implements sig (alphabet = char, answer = answer_type) {
	...
	answer_type f2 (answer_type p0, answer_type p1) {
		...
		string_type body;
		nt = getRuleName("rule1", result.shape);
		body = (("orig_f1 (" + ((getRuleName("rule2", p0.shape) + ", ") + getRuleName("rule7", p1.shape))) + ")");
		insertProduction (result, nt, body);
		return result;
	}
	...
	answer_type f7 (char p0, answer_type p1, char p2) {
...
grammar grmmr uses sig (axiom = rule00) {
	...
	rule00 = f00 (rule01);
	rule01 = rule0 # h;
	rule1 = f1 (rule2) | f2 (rule2, rule7) | f3 (EMPTY);
	rule2 = f4 (rule3) | f5 (rule6);
	rule3 = f6 (ROPE with matchString("[]"));
	rule6 = f7 (CHAR ('['), rule1, CHAR (']'));
	rule7 = f8 (ROPE with matchString("[]"));
}
