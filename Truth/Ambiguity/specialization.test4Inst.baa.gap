...
algebra alg implements sig (alphabet = char, answer = answer_type) {
	answer_type f0 (char p0, answer_type p1, char p2) {
		...
		body = (("orig_f3 (" + ((("CHAR, " + getRuleName("rule2", p1.shape)) + ", ") + "CHAR")) + ")");
		insertProduction (result, nt, body);
		nt = getRuleName("rule0", result.shape);
		body = (("orig_f1 (" + ((getRuleName("rule1", "") + ", ") + getRuleName("rule0", result.shape))) + ")");
		insertProduction (result, nt, body);
		return result;
	}
...
grammar grmmr uses sig (axiom = rule00) {
	rule0 = f0 (CHAR ('['), rule2, CHAR (']'));
	rule00 = f00 (rule01);
	rule01 = rule0 # h;
	rule1 = f1 (EMPTY);
	rule2 = f2 (CHAR ('['), rule0, CHAR (']')) | f3 (rule1);
}
