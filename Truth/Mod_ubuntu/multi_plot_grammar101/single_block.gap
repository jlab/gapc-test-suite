digraph stefansDangle {
compound = True;
newrank = True;
ordering = out;
subgraph cluster_1 {
    node_1 [ label=<<table border='0'><tr><td>struct</td></tr></table>>, color="black", shape="box", penwidth=3, style="dotted" ];
    node_1 -> node_2 [ style=invis, weight=99 ];
    node_2 [ label=<<table border='0'><tr><td><font point-size='30'>&rarr;</font></td></tr></table>>, shape=plaintext ];
    node_2 -> node_3 [ style=invis ];
    node_3 [ label=<<table border='0'><tr><td>root</td></tr></table>>, color="green" ];
    node_4 [ label=<<table border='0'><tr><td>hlcons</td></tr></table>>, color="black" ];
    node_3 -> node_4 [ arrowhead=none ];
    node_3 -> node_5 [ style=invis ];
    node_5 [ label=<<table border='0'><tr><td><font point-size='30'>|</font></td></tr></table>>, shape=plaintext ];
    node_5 -> node_6 [ style=invis ];
    node_6 [ label=<<table border='0'><tr><td>unpaired</td></tr></table>>, color="green" ];
    node_7 [ label=<<table border='0'><tr><td>REGION</td></tr></table>>, color="blue" ];
    node_6 -> node_7 [ arrowhead=none ];
    node_8 [ label=h, fontcolor="purple", shape=none ];
    node_1 -> node_8 [ arrowhead=none, color="purple", weight=99 ];
    { rank=same node_1 node_2 node_3 node_5 node_6 }
}
node_8 -> node_9 [ style=invis ];
subgraph cluster_2 {
    node_9 [ label=<<table border='0'><tr><td>hlcons</td></tr></table>>, color="black", shape="box" ];
    node_9 -> node_10 [ style=invis, weight=99 ];
    node_10 [ label=<<table border='0'><tr><td><font point-size='30'>&rarr;</font></td></tr></table>>, shape=plaintext ];
    node_10 -> node_11 [ style=invis ];
    node_11 [ label=<<table border='0'><tr><td>lasthlnoss</td></tr></table>>, color="green" ];
    node_12 [ label=<<table border='0'><tr><td>component</td></tr></table>>, color="black" ];
    node_11 -> node_12 [ arrowhead=none ];
    node_11 -> node_13 [ style=invis ];
    node_13 [ label=<<table border='0'><tr><td><font point-size='30'>|</font></td></tr></table>>, shape=plaintext ];
    node_13 -> node_14 [ style=invis ];
    node_14 [ label=<<table border='0'><tr><td>lasthlss</td></tr></table>>, color="green" ];
    node_15 [ label=<<table border='0'><tr><td>component</td></tr></table>>, color="black" ];
    node_14 -> node_15 [ arrowhead=none ];
    node_16 [ label=<<table border='0'><tr><td>REGION</td></tr></table>>, color="blue" ];
    node_14 -> node_16 [ arrowhead=none ];
    node_14 -> node_17 [ style=invis ];
    node_17 [ label=<<table border='0'><tr><td><font point-size='30'>|</font></td></tr></table>>, shape=plaintext ];
    node_17 -> node_18 [ style=invis ];
    node_18 [ label=<<table border='0'><tr><td>nexthl</td></tr></table>>, color="green" ];
    node_19 [ label=<<table border='0'><tr><td>component</td></tr></table>>, color="black" ];
    node_18 -> node_19 [ arrowhead=none ];
    node_20 [ label=<<table border='0'><tr><td>hlcons</td></tr></table>>, color="black" ];
    node_18 -> node_20 [ arrowhead=none ];
    node_21 [ label=h, fontcolor="purple", shape=none ];
    node_9 -> node_21 [ arrowhead=none, color="purple", weight=99 ];
    { rank=same node_9 node_10 node_11 node_13 node_14 node_17 node_18 }
}
node_21 -> node_22 [ style=invis ];
subgraph cluster_3 {
    node_22 [ label=<<table border='0'><tr><td>mlcons</td></tr></table>>, color="black", shape="box" ];
    node_22 -> node_23 [ style=invis, weight=99 ];
    node_23 [ label=<<table border='0'><tr><td><font point-size='30'>&rarr;</font></td></tr></table>>, shape=plaintext ];
    node_23 -> node_24 [ style=invis ];
    node_24 [ label=<<table border='0'><tr><td>lastmlnoss</td></tr></table>>, color="green" ];
    node_25 [ label=<<table border='0'><tr><td>component</td></tr></table>>, color="black" ];
    node_24 -> node_25 [ arrowhead=none ];
    node_26 [ label=<<table border='0'><tr><td>component</td></tr></table>>, color="black" ];
    node_24 -> node_26 [ arrowhead=none ];
    node_24 -> node_27 [ style=invis ];
    node_27 [ label=<<table border='0'><tr><td><font point-size='30'>|</font></td></tr></table>>, shape=plaintext ];
    node_27 -> node_28 [ style=invis ];
    node_28 [ label=<<table border='0'><tr><td>lastmlss</td></tr></table>>, color="green" ];
    node_29 [ label=<<table border='0'><tr><td>component</td></tr></table>>, color="black" ];
    node_28 -> node_29 [ arrowhead=none ];
    node_30 [ label=<<table border='0'><tr><td>component</td></tr></table>>, color="black" ];
    node_28 -> node_30 [ arrowhead=none ];
    node_31 [ label=<<table border='0'><tr><td>REGION</td></tr></table>>, color="blue" ];
    node_28 -> node_31 [ arrowhead=none ];
    node_28 -> node_32 [ style=invis ];
    node_32 [ label=<<table border='0'><tr><td><font point-size='30'>|</font></td></tr></table>>, shape=plaintext ];
    node_32 -> node_33 [ style=invis ];
    node_33 [ label=<<table border='0'><tr><td>nextml</td></tr></table>>, color="green" ];
    node_34 [ label=<<table border='0'><tr><td>component</td></tr></table>>, color="black" ];
    node_33 -> node_34 [ arrowhead=none ];
    node_35 [ label=<<table border='0'><tr><td>mlcons</td></tr></table>>, color="black" ];
    node_33 -> node_35 [ arrowhead=none ];
    node_36 [ label=h, fontcolor="purple", shape=none ];
    node_22 -> node_36 [ arrowhead=none, color="purple", weight=99 ];
    { rank=same node_22 node_23 node_24 node_27 node_28 node_32 node_33 }
}
node_36 -> node_37 [ style=invis ];
subgraph cluster_4 {
    node_37 [ label=<<table border='0'><tr><td>component</td></tr></table>>, color="black", shape="box" ];
    node_37 -> node_38 [ style=invis, weight=99 ];
    node_38 [ label=<<table border='0'><tr><td><font point-size='30'>&rarr;</font></td></tr></table>>, shape=plaintext ];
    node_38 -> node_39 [ style=invis ];
    node_39 [ label=<<table border='0'><tr><td>addRegion</td></tr></table>>, color="green" ];
    node_40 [ label=<<table border='0'><tr><td>REGION</td></tr></table>>, color="blue" ];
    node_39 -> node_40 [ arrowhead=none ];
    node_41 [ label=<<table border='0'><tr><td>initstem</td></tr></table>>, color="black" ];
    node_39 -> node_41 [ arrowhead=none ];
    node_39 -> node_42 [ style=invis ];
    node_42 [ label=<<table border='0'><tr><td><font point-size='30'>|</font></td></tr></table>>, shape=plaintext ];
    node_42 -> node_43 [ style=invis ];
    node_43 [ label=<<table border='0'><tr><td>startstem</td></tr></table>>, color="green" ];
    node_44 [ label=<<table border='0'><tr><td>initstem</td></tr></table>>, color="black" ];
    node_43 -> node_44 [ arrowhead=none ];
    node_45 [ label=h, fontcolor="purple", shape=none ];
    node_37 -> node_45 [ arrowhead=none, color="purple", weight=99 ];
    { rank=same node_37 node_38 node_39 node_42 node_43 }
}
node_45 -> node_46 [ style=invis ];
subgraph cluster_5 {
    node_46 [ label=<<table border='0'><tr><td>initstem</td></tr></table>>, color="black", shape="box", style="dotted" ];
    node_46 -> node_47 [ style=invis, weight=99 ];
    node_47 [ label=<<table border='0'><tr><td><font point-size='30'>&rarr;</font></td></tr></table>>, shape=plaintext ];
    node_47 -> node_48 [ style=invis ];
    node_48 [ label=<<table border='0'><tr><td>drem</td></tr></table>>, color="green" ];
    node_49 [ label=<<table border='0'><tr><td>LOC</td></tr></table>>, color="blue" ];
    node_48 -> node_49 [ arrowhead=none ];
    node_50 [ label=<<table border='0'><tr><td>stem</td></tr></table>>, color="black" ];
    node_48 -> node_50 [ arrowhead=none ];
    node_51 [ label=<<table border='0'><tr><td>LOC</td></tr></table>>, color="blue" ];
    node_48 -> node_51 [ arrowhead=none ];
    node_48 -> node_52 [ style=invis ];
    node_52 [ label=<<table border='0'><tr><td><font point-size='30'>|</font></td></tr></table>>, shape=plaintext ];
    node_52 -> node_53 [ style=invis ];
    node_53 [ label=<<table border='0'><tr><td>edlr</td></tr></table>>, color="green" ];
    node_54 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_53 -> node_54 [ arrowhead=none ];
    node_55 [ label=<<table border='0'><tr><td>stem</td></tr></table>>, color="black" ];
    node_53 -> node_55 [ arrowhead=none ];
    node_56 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_53 -> node_56 [ arrowhead=none ];
    node_53 -> node_57 [ style=invis ];
    node_57 [ label=<<table border='0'><tr><td><font point-size='30'>|</font></td></tr></table>>, shape=plaintext ];
    node_57 -> node_58 [ style=invis ];
    node_58 [ label=<<table border='0'><tr><td>edl</td></tr></table>>, color="green" ];
    node_59 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_58 -> node_59 [ arrowhead=none ];
    node_60 [ label=<<table border='0'><tr><td>stem</td></tr></table>>, color="black" ];
    node_58 -> node_60 [ arrowhead=none ];
    node_61 [ label=<<table border='0'><tr><td>LOC</td></tr></table>>, color="blue" ];
    node_58 -> node_61 [ arrowhead=none ];
    node_58 -> node_62 [ style=invis ];
    node_62 [ label=<<table border='0'><tr><td><font point-size='30'>|</font></td></tr></table>>, shape=plaintext ];
    node_62 -> node_63 [ style=invis ];
    node_63 [ label=<<table border='0'><tr><td>edr</td></tr></table>>, color="green" ];
    node_64 [ label=<<table border='0'><tr><td>LOC</td></tr></table>>, color="blue" ];
    node_63 -> node_64 [ arrowhead=none ];
    node_65 [ label=<<table border='0'><tr><td>stem</td></tr></table>>, color="black" ];
    node_63 -> node_65 [ arrowhead=none ];
    node_66 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_63 -> node_66 [ arrowhead=none ];
    node_67 [ label=h, fontcolor="purple", shape=none ];
    node_46 -> node_67 [ arrowhead=none, color="purple", weight=99 ];
    { rank=same node_46 node_47 node_48 node_52 node_53 node_57 node_58 node_62 node_63 }
}
node_67 -> node_68 [ style=invis ];
subgraph cluster_6 {
    node_68 [ label=<<table border='0'><tr><td>stem</td></tr></table>>, color="black", shape="box" ];
    node_68 -> node_69 [ style=invis, weight=99 ];
    node_69 [ label=<<table border='0'><tr><td><font point-size='30'>&rarr;</font></td></tr></table>>, shape=plaintext ];
    node_69 -> node_70 [ style=invis ];
    node_70 [ label=<<table border='0'><tr><td>a block</td></tr></table>>, color="gray" ];
    node_71 [ label=<<table border='0'><tr><td>stack</td></tr></table>>, color="green" ];
    node_72 [ label="basepairing" , fontcolor="magenta" , shape=none ];
    node_71 -> node_72 [ arrowhead=none, color="magenta" ];
    node_73 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_71 -> node_73 [ arrowhead=none ];
    node_74 [ label=<<table border='0'><tr><td>stem</td></tr></table>>, color="black" ];
    node_71 -> node_74 [ arrowhead=none ];
    node_75 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_71 -> node_75 [ arrowhead=none ];
    node_70 -> node_71 [ ];
    node_70 -> node_76 [ style=invis ];
    node_76 [ label=<<table border='0'><tr><td><font point-size='30'>|</font></td></tr></table>>, shape=plaintext ];
    node_76 -> node_77 [ style=invis ];
    node_77 [ label=<<table border='0'><tr><td>a block</td></tr></table>>, color="gray" ];
    node_78 [ label=<<table border='0'><tr><td>hairpin</td></tr></table>>, color="green" ];
    node_79 [ label="stackpairing" , fontcolor="magenta" , shape=none ];
    node_78 -> node_79 [ arrowhead=none, color="magenta" ];
    node_80 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_78 -> node_80 [ arrowhead=none ];
    node_81 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_78 -> node_81 [ arrowhead=none ];
    node_82 [ label=<<table border='0'><tr><td>REGION</td></tr></table>>, color="blue" ];
    node_83 [ label="minsize(3)" , fontcolor="magenta" , shape=none ];
    node_82 -> node_83 [ arrowhead=none, color="magenta" ];
    node_78 -> node_82 [ arrowhead=none ];
    node_84 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_78 -> node_84 [ arrowhead=none ];
    node_85 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_78 -> node_85 [ arrowhead=none ];
    node_77 -> node_78 [ ];
    node_77 -> node_86 [ style=invis ];
    node_86 [ label=<<table border='0'><tr><td><font point-size='30'>|</font></td></tr></table>>, shape=plaintext ];
    node_86 -> node_87 [ style=invis ];
    node_87 [ label=<<table border='0'><tr><td>a block</td></tr></table>>, color="gray" ];
    node_88 [ label=<<table border='0'><tr><td>bulgeleft</td></tr></table>>, color="green" ];
    node_89 [ label="stackpairing" , fontcolor="magenta" , shape=none ];
    node_88 -> node_89 [ arrowhead=none, color="magenta" ];
    node_90 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_88 -> node_90 [ arrowhead=none ];
    node_91 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_88 -> node_91 [ arrowhead=none ];
    node_92 [ label=<<table border='0'><tr><td>REGION</td></tr></table>>, color="blue" ];
    node_88 -> node_92 [ arrowhead=none ];
    node_93 [ label=<<table border='0'><tr><td>stem</td></tr></table>>, color="black" ];
    node_88 -> node_93 [ arrowhead=none ];
    node_94 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_88 -> node_94 [ arrowhead=none ];
    node_95 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_88 -> node_95 [ arrowhead=none ];
    node_87 -> node_88 [ ];
    node_87 -> node_96 [ style=invis ];
    node_96 [ label=<<table border='0'><tr><td><font point-size='30'>|</font></td></tr></table>>, shape=plaintext ];
    node_96 -> node_97 [ style=invis ];
    node_97 [ label=<<table border='0'><tr><td>a block</td></tr></table>>, color="gray" ];
    node_98 [ label=<<table border='0'><tr><td>bulgeright</td></tr></table>>, color="green" ];
    node_99 [ label="stackpairing" , fontcolor="magenta" , shape=none ];
    node_98 -> node_99 [ arrowhead=none, color="magenta" ];
    node_100 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_98 -> node_100 [ arrowhead=none ];
    node_101 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_98 -> node_101 [ arrowhead=none ];
    node_102 [ label=<<table border='0'><tr><td>stem</td></tr></table>>, color="black" ];
    node_98 -> node_102 [ arrowhead=none ];
    node_103 [ label=<<table border='0'><tr><td>REGION</td></tr></table>>, color="blue" ];
    node_98 -> node_103 [ arrowhead=none ];
    node_104 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_98 -> node_104 [ arrowhead=none ];
    node_105 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_98 -> node_105 [ arrowhead=none ];
    node_97 -> node_98 [ ];
    node_97 -> node_106 [ style=invis ];
    node_106 [ label=<<table border='0'><tr><td><font point-size='30'>|</font></td></tr></table>>, shape=plaintext ];
    node_106 -> node_107 [ style=invis ];
    node_107 [ label=<<table border='0'><tr><td>a block</td></tr></table>>, color="gray" ];
    node_108 [ label=<<table border='0'><tr><td>iloop</td></tr></table>>, color="green" ];
    node_109 [ label="stackpairing" , fontcolor="magenta" , shape=none ];
    node_108 -> node_109 [ arrowhead=none, color="magenta" ];
    node_110 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_108 -> node_110 [ arrowhead=none ];
    node_111 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_108 -> node_111 [ arrowhead=none ];
    node_112 [ label=<<table border='0'><tr><td>REGION</td></tr></table>>, color="blue" ];
    node_113 [ label="maxsize(30)" , fontcolor="magenta" , shape=none ];
    node_112 -> node_113 [ arrowhead=none, color="magenta" ];
    node_108 -> node_112 [ arrowhead=none ];
    node_114 [ label=<<table border='0'><tr><td>stem</td></tr></table>>, color="black" ];
    node_108 -> node_114 [ arrowhead=none ];
    node_115 [ label=<<table border='0'><tr><td>REGION</td></tr></table>>, color="blue" ];
    node_116 [ label="maxsize(30)" , fontcolor="magenta" , shape=none ];
    node_115 -> node_116 [ arrowhead=none, color="magenta" ];
    node_108 -> node_115 [ arrowhead=none ];
    node_117 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_108 -> node_117 [ arrowhead=none ];
    node_118 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_108 -> node_118 [ arrowhead=none ];
    node_107 -> node_108 [ ];
    node_107 -> node_119 [ style=invis ];
    node_119 [ label=<<table border='0'><tr><td><font point-size='30'>|</font></td></tr></table>>, shape=plaintext ];
    node_119 -> node_120 [ style=invis ];
    node_120 [ label=<<table border='0'><tr><td>a block</td></tr></table>>, color="gray" ];
    node_121 [ label=<<table border='0'><tr><td>multiloop_drem</td></tr></table>>, color="green" ];
    node_122 [ label="stackpairing" , fontcolor="magenta" , shape=none ];
    node_121 -> node_122 [ arrowhead=none, color="magenta" ];
    node_123 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_121 -> node_123 [ arrowhead=none ];
    node_124 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_121 -> node_124 [ arrowhead=none ];
    node_125 [ label=<<table border='0'><tr><td>mlcons</td></tr></table>>, color="black" ];
    node_121 -> node_125 [ arrowhead=none ];
    node_126 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_121 -> node_126 [ arrowhead=none ];
    node_127 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_121 -> node_127 [ arrowhead=none ];
    node_120 -> node_121 [ ];
    node_120 -> node_128 [ style=invis ];
    node_128 [ label=<<table border='0'><tr><td><font point-size='30'>|</font></td></tr></table>>, shape=plaintext ];
    node_128 -> node_129 [ style=invis ];
    node_129 [ label=<<table border='0'><tr><td>a block</td></tr></table>>, color="gray" ];
    node_130 [ label=<<table border='0'><tr><td>multiloop_edlr</td></tr></table>>, color="green" ];
    node_131 [ label="stackpairing" , fontcolor="magenta" , shape=none ];
    node_130 -> node_131 [ arrowhead=none, color="magenta" ];
    node_132 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_130 -> node_132 [ arrowhead=none ];
    node_133 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_130 -> node_133 [ arrowhead=none ];
    node_134 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_130 -> node_134 [ arrowhead=none ];
    node_135 [ label=<<table border='0'><tr><td>mlcons</td></tr></table>>, color="black" ];
    node_130 -> node_135 [ arrowhead=none ];
    node_136 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_130 -> node_136 [ arrowhead=none ];
    node_137 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_130 -> node_137 [ arrowhead=none ];
    node_138 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_130 -> node_138 [ arrowhead=none ];
    node_129 -> node_130 [ ];
    node_129 -> node_139 [ style=invis ];
    node_139 [ label=<<table border='0'><tr><td><font point-size='30'>|</font></td></tr></table>>, shape=plaintext ];
    node_139 -> node_140 [ style=invis ];
    node_140 [ label=<<table border='0'><tr><td>a block</td></tr></table>>, color="gray" ];
    node_141 [ label=<<table border='0'><tr><td>multiloop_edl</td></tr></table>>, color="green" ];
    node_142 [ label="stackpairing" , fontcolor="magenta" , shape=none ];
    node_141 -> node_142 [ arrowhead=none, color="magenta" ];
    node_143 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_141 -> node_143 [ arrowhead=none ];
    node_144 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_141 -> node_144 [ arrowhead=none ];
    node_145 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_141 -> node_145 [ arrowhead=none ];
    node_146 [ label=<<table border='0'><tr><td>mlcons</td></tr></table>>, color="black" ];
    node_141 -> node_146 [ arrowhead=none ];
    node_147 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_141 -> node_147 [ arrowhead=none ];
    node_148 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_141 -> node_148 [ arrowhead=none ];
    node_140 -> node_141 [ ];
    node_140 -> node_149 [ style=invis ];
    node_149 [ label=<<table border='0'><tr><td><font point-size='30'>|</font></td></tr></table>>, shape=plaintext ];
    node_149 -> node_150 [ style=invis ];
    node_150 [ label=<<table border='0'><tr><td>a block</td></tr></table>>, color="gray" ];
    node_151 [ label=<<table border='0'><tr><td>multiloop_edr</td></tr></table>>, color="green" ];
    node_152 [ label="stackpairing" , fontcolor="magenta" , shape=none ];
    node_151 -> node_152 [ arrowhead=none, color="magenta" ];
    node_153 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_151 -> node_153 [ arrowhead=none ];
    node_154 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_151 -> node_154 [ arrowhead=none ];
    node_155 [ label=<<table border='0'><tr><td>mlcons</td></tr></table>>, color="black" ];
    node_151 -> node_155 [ arrowhead=none ];
    node_156 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_151 -> node_156 [ arrowhead=none ];
    node_157 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_151 -> node_157 [ arrowhead=none ];
    node_158 [ label=<<table border='0'><tr><td>BASE</td></tr></table>>, color="blue" ];
    node_151 -> node_158 [ arrowhead=none ];
    node_150 -> node_151 [ ];
    node_159 [ label=h, fontcolor="purple", shape=none ];
    node_68 -> node_159 [ arrowhead=none, color="purple", weight=99 ];
    node_159 -> node_160 [ style=invis, weight=99 ];
    node_160 [ style=invis ];
    node_160 -> node_161 [ style=invis, weight=99 ];
    node_161 [ style=invis ];
    { rank=same node_68 node_69 node_70 node_76 node_77 node_86 node_87 node_96 node_97 node_106 node_107 node_119 node_120 node_128 node_129 node_139 node_140 node_149 node_150 }
}
}
